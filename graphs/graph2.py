...
class Nav17ToxinGraphAnalyzer:
    ...
    def analyze_single_toxin(self, peptide_code, cutoff_distance=8.0, plot_3d=False):
        """
        Análisis estructural de una toxina por código de péptido, mostrando información simplificada
        relevante para la interacción con Nav1.7
        
        Args:
            peptide_code (str): Código del péptido a analizar
            cutoff_distance (float): Distancia de corte para interacciones entre residuos
            plot_3d (bool): Parámetro ignorado, siempre se usa visualización 2D para evitar errores
        """
        try:
            print(f"=== Análisis de toxina {peptide_code} ===")
            
            # 1. Obtención de patrón farmacofórico e información básica
            pharmacophore_pattern = None
            conn = self._connect_db()
            cursor = conn.cursor()
            cursor.execute("SELECT pharmacophore_match, sequence FROM Nav1_7_InhibitorPeptides WHERE peptide_code = ?", 
                        (peptide_code,))
            result = cursor.fetchone()
            conn.close()
            
            if result:
                pharmacophore_pattern, db_sequence = result
            ...
            # 8. Mostrar motivos estructurales completos
